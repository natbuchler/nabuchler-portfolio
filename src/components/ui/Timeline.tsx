 ...
 * Timeline Components
 *
 * Modular timeline system for displaying chronological experience:
 * - Timeline: Container with vertical line background
 * - TimelineItem: Individual timeline items with icon and content wrapper
 *
 * Features:
 * - Vertical connecting line with SVG pattern
 * - Customizable icons with color control
...
export interface TimelineItemProps {
  ...
  iconColor: string;
  /** Content to display in the timeline item */
...
/**
 * Individual timeline item with icon and content wrapper
 */
export function TimelineItem({ icon, iconColor, children, index }: TimelineItemProps) {
  const imgCy = "/cy.svg";
...
export interface TimelineProps {
  ...
  children: React.ReactNode;
  /** Path to vertical line SVG image */
  lineImage: string;
}

/**
 * Timeline container with vertical connecting line
 * Uses dual rendering approach (CSS background + Image) for reliability
 */
export default function Timeline({ children, lineImage }: TimelineProps) {
  return (
    <div className="space-y-12 flex flex-col items-start relative max-w-6xl mx-auto">
      {/* Vertical timeline line - CSS background approach */}
      <div
      ...
      {/* Fallback with Next.js Image component */}
      ...
      {/* Timeline items with higher z-index */}
      {children}
    </div>
  );
}
